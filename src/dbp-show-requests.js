...
import * as cabinetStyles from './styles';
...
class ShowRequests extends ScopedElementsMixin(DBPCabinetLitElement) {
    constructor() {
        super();
    ...
    disconnectedCallback() {
        this.cabinetRequestsTable.off("rowClick");
        this.cabinetRequestsTable.off("dataLoaded");
        this.cabinetRequestsTable.off("pageLoaded");
    ...
    connectedCallback() {
        ...
        this.updateComplete.then(() => {
            ...
            this.cabinetRequestsTable = new Tabulator(this._('#cabinet-requests-table'), {
                layout: 'fitColumns',
                ...
                columns: [
                    {
                        ...
                        headerClick: (e) => {
                            ...
                            if (allSelected) {
                                // that.cabinetRequestsTable.deselectRow("visible"));
                                this.cabinetRequestsTable.deselectRow();
                                this._('#select_all').checked = false;
                                this.rowsSelected = false;
                            } else {
                                that.cabinetRequestsTable.selectRow("visible");
            ...
            this.cabinetRequestsTable.on("rowClick", this.rowClickFunction.bind(this));
            //this.cabinetRequestsTable.on("rowAdded", this.rowAddedFunction.bind(this));
            this.cabinetRequestsTable.on("dataLoaded", this.dataLoadedFunction.bind(this));
            this.cabinetRequestsTable.on("pageLoaded", this.pageLoadedFunction.bind(this));
    ...
    clearFilter() {
        ...
        if (!filter || !search || !this.cabinetRequestsTable)
            return;

        filter.value = '';
        search.value = 'all';
        this.cabinetRequestsTable.clearFilter();
    ...
    filterTable() {
        ...
        if (!filter || !operator || !search || !this.cabinetRequestsTable)
            return;
        ...
        if (search !== 'all') {
            this.cabinetRequestsTable.setFilter(search, operator, filter);
            return;
        }

        let filterArray = [];
        this.cabinetRequestsTable.getColumns().forEach((col) => {
            let field = col.getDefinition().field;
            filterArray.push({field: field, type: operator, value: filter});
        });

        this.cabinetRequestsTable.setFilter([filterArray]);
        this.totalNumberOfItems = this.cabinetRequestsTable.getDataCount("active");
    }
    ...
    getTableHeaderOptions() {
        ...
        this.cabinetRequestsTable.getColumns().forEach((col, counter) => {
    ...
    static get styles() {
        ...
        return css`
            ...
            ${cabinetStyles.getCabinetRequestTableStyles()}
            ${cabinetStyles.getCabinetRequestStyles()}
        ...
        return html`
            ...
            <div class="${classMap({hidden: !this.isLoggedIn() || this.isLoading() || this.loadingTranslations})}">
                ...
                <h3 class="${classMap({hidden: !this.isLoggedIn() || this.isLoading() || this.showDetailsView || !this.organizationSet || this.loadingTranslations})}">
                    ${i18n.t('show-requests.cabinet-orders')}
                </h3>
                ...
                <div class="${classMap({hidden: !this.isLoggedIn() || this.isLoading() || this.loadingTranslations|| this.showDetailsView || !this.organizationSet || (!this.mayRead && !this.mayReadMetadata)})}">
                    <div class="table-wrapper">
                        <div class="selected-buttons">
                            ...
                            <div class="cabinet-table ${classMap({hidden: !this.isLoggedIn() || this.isLoading() || this.loadingTranslations || this.showDetailsView || this.initialRequestsLoading || this.tableLoading })}">
                                <div id="cabinet-requests-table" class=""></div>
                ...
                ${ this.mayRead || this.mayReadMetadata ? html`
                    <div class="back-container">
                        <span class="back-navigation ${classMap({hidden: !this.isLoggedIn() || this.isLoading() || this.loadingTranslations || this.showListView || !this.organizationSet })}">
                            <a href="#" title="${i18n.t('show-requests.back-to-list')}"
                               @click="${() => {
                                   let currentPage = this.cabinetRequestsTable ? this.cabinetRequestsTable.getPage() : 1;
                                   this.getListOfRequests().then(() => {
                                       this.cabinetRequestsTable ? this.cabinetRequestsTable.setPage(currentPage) : null;
                                   });
                                   this.showListView = true;
                                   this.showDetailsView = false;
                                   this.currentItem = {};
                    ...
                    <h3 class="${classMap({hidden: !this.isLoggedIn() || this.isLoading() || this.loadingTranslations || this.showListView || !this.organizationSet })}">
                        ${(this.currentItem && this.currentItem.dateSubmitted) || !this.mayWrite ? 
                               i18n.t('show-requests.show-detailed-cabinet-order', { id: this.currentItem.identifier }) 
                               : i18n.t('show-requests.detailed-cabinet-order', { id: this.currentItem.identifier })
                        }:
                    </h3>
